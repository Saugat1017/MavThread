...
// You can keep this if you also support token-based auth (optional)
function getToken() {
    return localStorage.getItem('jwt') // used only if needed
}
...
async function request(path, { method = 'GET', body, params, headers = {} } = {}) {
    ...
    // 🔐 OPTIONAL: only needed if using Authorization header
    ...
    // read full body as text exactly once
    const txt = await res.text()

    // try JSON—if it fails, just return the raw text
    let data
    try {
        data = JSON.parse(txt)
    } catch {
...
export function logout() {
    localStorage.removeItem('jwt') // only needed if you ever store token manually
}
